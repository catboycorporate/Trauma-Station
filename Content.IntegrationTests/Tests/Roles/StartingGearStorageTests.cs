...
public sealed class StartingGearPrototypeStorageTest
{
    ...
    public async Task TestStartingGearStorage()
    {
        ...
        var protos = server.ProtoMan
            ...
            .Where(p => !p.Abstract); // Trauma - removed ToList and OrderBy, no reason to do that
        ...
        {
            // <Trauma> - wrapped in Assert.Multiple
            Assert.Multiple(() =>
            {
                    ...
                    foreach (var (slot, entProtos) in gearProto.Storage)
                    {
                        ...
                        foreach (var ent in ents)
                        {
                            if (!storageSystem.CanInsert(bag, ent, out _))
                                Assert.Fail($"StartingGearPrototype {gearProto.ID} could not successfully put {server.EntMan.ToPrettyString(ent)} into storage {server.EntMan.ToPrettyString(bag)} from slot {slot}"); // Trauma - improved output
                        }
                        // <Trauma> - delete the entities after it's checked that they can all be inserted together
                        // otherwise it would only check that items can individually fit
                        foreach (var ent in ents)
                        {
