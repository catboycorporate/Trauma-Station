...
public sealed class AdjustPainFeelsEffectSystem : EntityEffectSystem<BodyComponent, AdjustPainFeels>
{
    ...
    protected override void Effect(Entity<BodyComponent> ent, ref EntityEffectEvent<AdjustPainFeels> args)
    {
        ...
        foreach (var bodyPart in _body.GetBodyChildren(ent))
        {
            var add = _random.Prob(0.3f) ? amount : -amount;
            if (_pain.TryGetPainFeelsModifier(bodyPart.Id, nerves, ident, out var modifier))
                _pain.TryChangePainFeelsModifier(nerves, ident, bodyPart.Id, add);
            else
                _pain.TryAddPainFeelsModifier(nerves, ident, bodyPart.Id, add);
        }
    }
}
